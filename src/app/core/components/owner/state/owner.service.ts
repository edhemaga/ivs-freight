        ...
        const subOwner = this.getOwnerById(data.id).subscribe({
          next: (owner: OwnerResponse | any) => {
            if (!owner.truckCount && !owner.trailerCount) {
              this.ownerInactiveStore.remove(({ id }) => id === data.id);

              this.ownerInactiveStore.add(owner);
            } else {
              this.ownerActiveStore.remove(({ id }) => id === data.id);
      ...
      tap(() => {
        ...
        storeOwners.map((owner: any) => {
          deleteOnBack.map((d) => {
            if (d === owner.id) {
              selectedTab === 'active'
                ? this.ownerActiveStore.remove(({ id }) => id === owner.id)
                : this.ownerInactiveStore.remove(({ id }) => id === owner.id);
            }
          });
        });
