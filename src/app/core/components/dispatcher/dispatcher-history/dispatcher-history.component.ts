...
export class DispatcherHistoryComponent implements OnInit {
    ...
    boardChanged(board?: any): void {
        ...
        params &&
            this.getDispatchBoardHistoryMainData(params ? `?${params}` : '');
    }
                ...
                this.historyBoardList = res.reduce(
                    (data, item, indx) => {
                        if (
                            ...
                            data.trailerList[data.trailerList.length - 1][
                                'heightCount'
                            ] += 1;
                        else
                        ...
                        if (
                            res[indx - 1] &&
                            res[indx - 1]['driverId'] == item.driverId
                        )
                            data.driversList[data.driversList.length - 1][
                                'heightCount'
                            ] += 1;
                        else
                        ...
                        data.statusList.push(
                            this.dispatcBoardStatuses.find(
                                (it) => it.id == item.statusId
                            )
                        );
    ...
    saveDate(): void {
        if (this.pickupChangeDate) {
            ...
            const type = this.selectedDriver
                ? this.historyBoardList['pickupDates'][
            ...
            switch (this.selectedPicker.type) {
                case 'pickupDate':
                    ...
                    this.sendStatusChangeData(
                        { startDate: this.pickupChangeDate },
                        type
                    );
                    break;
                case 'pickupTime':
                    this.historyBoardList['pickupTimes'][
                        this.selectedPicker.index
                    ].date = selectedDate;
                    this.sendStatusChangeData(
                        {
                            startTime: moment(this.pickupChangeDate).format(
                                'HH:mm:ss'
                            ),
                        },
                        type
                ...
                default:
                    ...
                    this.sendStatusChangeData(
                        {
                            endTime: moment(this.pickupChangeDate).format(
                                'HH:mm:ss'
                            ),
                        },
                        type
