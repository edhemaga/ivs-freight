        ...
        const allwaypoints = mod.route.reduce(
            (routeArray, item) => {
                if (
                    ...
                    (item.PointType == 'delivery' &&
                        item.PointOrder < mod.deliveryCount)
                ) {
                    routeArray.routeAddress.push({
                        location: item.PointAddress,
                    });
    ...
    public getDispatchMapDistance(
        ...
        directionsService.route(request, (response, status) => {
            if (status === google.maps.DirectionsStatus.OK) {
                ...
                this.distance = (totalDistance * 0.00062137)
                    .toFixed(0)
                    .toString();
                this.legMilage.forEach((element, index) => {
                    if (index === 0) {
                        ...
                        element.mileageSum =
                            ...
                            this.legMilage[index - 1].mileageSum;
                        element.mileageSumString = (
                            element.mileageSum * 0.00062137
                        )
                            .toFixed(0)
                            .toString();
                    }
