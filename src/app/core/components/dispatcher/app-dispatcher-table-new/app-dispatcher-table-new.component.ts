...
export class AppDispatcherTableNewComponent implements OnInit, OnChanges {
    ...
    ngOnInit(): void {
            ...
            this.signalRService.startConnection().then((res) => {
                ...
                this.signalRService.statusChange
                    ...
                    .subscribe((data) => {
                        data.map((item) => {
                            if (item.hosJson) {
                                item.hosJson = JSON.parse(item.hosJson);
                            }
                            if (item.deliveryJson) {
                                item.deliveryJson = JSON.parse(
                                    item.deliveryJson
                                );
                            }
                            if (item.pickupJson) {
                                item.pickupJson = JSON.parse(item.pickupJson);
                            }
                            if (item.driverAddress) {
                                item.driverAddress = JSON.parse(
                                    item.driverAddress
                                );
                            }
                            if (item.route) {
        ...
        this.loadService
            .getDispatchData(this.dispatcherInfo.id)
            .pipe(takeUntil(this.destroy$))
            .subscribe((result: any) => {
                ...
                this.gridData.forEach((el) => {
                    if (el.truckId) {
                        this.infoCount.truckNumber += 1;
                    }
                    if (el.trailerId) {
                        this.infoCount.trailerNumber += 1;
                    }
                    if (el.driverId) {
                        this.infoCount.driverNumber += 1;
                    }
                ...
                // this.gridData = new Array(500).fill({}).map((item) => {
                //   item = result[0];
                //   return item;
                // });
                ...
                if (!this.isBoardLocked && this.gridData.length == 0)
                    this.gridData.push(
                        JSON.parse(JSON.stringify(this.emptyRow))
                    );
                else if (
                    !this.isBoardLocked &&
                    this.gridData[this.gridData.length - 1].id
                )
                    this.gridData.push(
                        JSON.parse(JSON.stringify(this.emptyRow))
                    );
        ...
        this.shared.emitDeleteAction
            ...
            .subscribe((resp: any) => {
                if (
                    resp.data.type == 'remove-item' &&
                    resp.data.index == this.gridIndex
                ) {
                    this.waitForResponse = true;
                    this.loadService
                        .removeDispatchTuckList(resp.data.id)
                        .subscribe(
                            () => {
    ...
    ngOnChanges() {
        if (!this.isBoardLocked && !this.highlightingWords.length) {
            if (this.gridData.length == 0)
                this.gridData.push(JSON.parse(JSON.stringify(this.emptyRow)));
            else if (this.gridData[this.gridData.length - 1].id)
                this.gridData.push(JSON.parse(JSON.stringify(this.emptyRow)));
        } else {
    ...
    public getDistance(origins: string, destinations: string, waypoint: any[]) {
        ...
        directionsService.route(request, (response, status) => {
            if (status === google.maps.DirectionsStatus.OK) {
                ...
                this.distance =
                    (totalDistance * 0.00062137).toFixed(0).toString() +
                    'miles';
            }
        });
    }
    ...
    addTruck(event, id, elem, rowInd?, itemType?) {
        ...
        const svg_placeholder_image =
            document.querySelector('.svg-placeholder');
        if (svg_placeholder_image) {
            svg_placeholder_image.classList.add('hidden');
        }
        if (rowInd !== null) {
            this.statusAfterAddingLocation = {
                truckId: event,
                dispatchId: id,
                changedRow: rowInd,
                type: 'truck',
            ...
            setTimeout(() => {
                if (this.newLocationField) {
                    this.newLocationField.nativeElement.focus();
                }
            }, 50);
    ...
    updateDispatchBoard(event, item, id) {
        ...
        if (item === 'truck') {
            ...
            if (event && event.location.city) {
                ...
                data.location =
                    event.location.city + ', ' + event.location.stateShortName;
            }
        ...
        this.waitForResponse = true;
        this.loadService
            .editDispatchBoardData(JSON.stringify(data), id)
            .pipe(takeUntil(this.destroy$))
            .subscribe(
                (result: any) => {
                ...
                .subscribe((res) => {
                    ...
                    res.forEach((elem: any, index: any) => {
                        if (!elem.error) {
                            ...
                            this.savedMainGridData.map(
                                (item: any, index: number) => {
                                    if (elem.id == item.id) {
                                        const trcol =
                                            elem.truckColor?.toString();
                                        elem.truckColor =
                                            trcol?.toLocaleLowerCase();
                            ...
                            this.savedMainGridData =
                                this.savedMainGridData.filter(
                                    (item) => item.id != final_id
                                );
                        }
                    ...
                    this.filterItemData();
                    this.signalRService.broadcastDbStatusData(
                        this.savedMainGridData.filter((item) => item.id)
                    );
                    if (fromSignal) {
    ...
    customSearchFn(term: string, item: any) {
        ...
        } else if (item.trailerNumber) {
            return item.trailerNumber.toLocaleLowerCase().indexOf(term) > -1;
        } else if (item.driverFirstName) {
    ...
    public findLocationInRoutes(
        rowIndx: number,
        type: string,
        indx: number = 1
    ) {
        if (!this.gridData[rowIndx].route) {
            return;
        }
        ...
        if (findedItem) {
            return JSON.parse(JSON.stringify(findedItem));
        } else {
            return {
                pointCity: '',
                pointState: '',
    ...
    addStatus(event: any, item: any, rowInd: number) {
        ...
        const data: any = {
            statusId: event.id,
            statusCounter: event.load_count,
        };
        if ([3000, 3100, 3200, 4000].includes(event.id)) {
            const load_count =
                event.id == 4000 ? item.pickupNumber : event.load_count;

            if ([3000, 3100].includes(event.id)) {
                const finded_route = JSON.parse(
                    JSON.stringify(
                        this.findLocationInRoutes(rowInd, 'pickup', load_count)
                    )
                );
                data.inDateTime = new Date();
                data.pointCity = finded_route.pointCity;
                data.pointState = finded_route.pointState;
                data.routeId = finded_route.pointId;
            }

            this.waitForResponse = true;
            this.loadService
                .updateDispatchStatus(JSON.stringify(data), item.id)
                .pipe(takeUntil(this.destroy$))
                .subscribe(
                    (result: any) => {
        ...
        if ([5000, 5100, 5200, 6000].includes(event.id)) {
            const load_count =
                event.id == 6000 ? item.deliveryNumber : event.load_count;
        ...
        if (
            (event.id === 4070 || event.id === 7000 || event.id == 8000) &&
            rowInd !== null
        ) {
            this.statusAfterAddingLocation = {
                statusId: event.id,
    ...
    public startingPointDeadHeadingDistance(location, destination) {
        return new Promise((resolve, reject) => {
            new google.maps.DistanceMatrixService().getDistanceMatrix(
                ...
                (results: any) => {
                    if (
                        results.rows[0].elements[0].status == 'NOT_FOUND' ||
                        results.rows[0].elements[0].status == 'ZERO_RESULTS'
                    ) {
                        resolve(0);
                    } else {
                        resolve(
    ...
    addStatusWithLocation(location: any) {
        if (this.statusAfterAddingLocation) {
            if (this.statusAfterAddingLocation.type == 'truck') {
                ...
                this.addTruckWithLocation(
                    this.statusAfterAddingLocation.dispatchId,
                    this.statusAfterAddingLocation.truckId
                );
                delete this.stopLocationAddress;
                return;
            }
            const data: any = {
                statusId: this.statusAfterAddingLocation.statusId,
                pointCity: location.city,
                pointState: location.stateShortName,
            };
            delete this.stopLocationAddress;
            this.waitForResponse = true;
            this.startingPointDeadHeadingDistance(
                this.truckLocationPosition,
                this.gridData[this.statusAfterAddingLocation.changedRow]
                    .location
            ).then((res) => {
    ...
    handleDispatchError(err?, showMsg?) {
        // this.errorBtn = true;
        const errorMessage =
            err && err.error ? err.error : 'Something went wrong.';
        if (!showMsg) {
    ...
    locationChangeBlur(rowInd: number) {
        setTimeout(() => {
            ...
            this.gridData[rowInd].statusId =
                this.statusAfterAddingLocation.statusId;
            if (this.gridData[rowInd].id) {
                this.retreveDispatchChanges([this.gridData[rowInd].id]);
            } else
                this.gridData[rowInd] = JSON.parse(
    ...
    public handleAddressChange(address: any, key?: any) {
        if (address.address_components) {
            ...
            if (!full_address) {
                full_address = this.stopLocationAddress;
            }
    ...
    updateLocationChanges(address: any) {
        if (this.statusAfterAddingLocation) {
            delete this.gridData[this.statusAfterAddingLocation.changedRow]
                .enterNewAddress;
            this.truckLocationPosition =
                this.gridData[
                    this.statusAfterAddingLocation.changedRow
                ].location;
            this.gridData[this.statusAfterAddingLocation.changedRow].location =
                address.city + ', ' + address.stateShortName;
        }
    }
    ...
    toggleLoadPopup(tooltip: any, data: any) {
        if (tooltip.isOpen()) {
            ...
            this.onTabChanged();
            this.loadService
                .getByDispatchboardData(data.id)
                .pipe(takeUntil(this.destroy$))
                .subscribe((res) => {
    ...
    addDispatchItem(event, item, isTrue?, dispatchId?) {
        ...
        if (this.dispatcher !== 0 && this.dispatcher !== undefined) {
            if (this.dispatcher < 0) {
                ...
                data.dispatcherId = this.dispatcher;
            }
        }

        this.loadService
            .addDispatchItem(JSON.stringify(data))
            .pipe(takeUntil(this.destroy$))
            .subscribe(
                (result: any) => {
    ...
    dropDriver(event: CdkDragDrop<string[]>, rowIndex) {
        ...
        const firstDriverName: any =
            this.gridData[this.startIndexDriver].driverName;
        ...
        if (this.gridData[rowIndex].id === null) {
            ...
            if (removeItem) {
                this.waitForResponse = true;
                this.loadService
                    .removeDispatchTuckList(dispatchItemId)
                    .pipe(takeUntil(this.destroy$))
                    .subscribe(
                        () => {
        ...
        if (firstEl === secondEl || this.isBoardLocked) {
            return;
        }
    ...
    dropTrailer(event: CdkDragDrop<string[]>, rowIndex) {
        ...
        const firstTrailerNumber: any =
            this.gridData[this.startIndexTrailer].trailerNumber;
        const trailerColor: any =
            this.gridData[this.startIndexTrailer].trailerColor;
    ...
    cdkDropListEnteredTrailer(event) {
        ...
        event.container.element.nativeElement.style.transition =
            'all 0.5s ease';
    }
    ...
    dropList(event) {
        ...
        if (event.currentIndex === this.gridData.length - 1) {
            return;
        }
        ...
        this.loadService
            ...
            .subscribe(
                () => {
                    this.retreveDispatchChanges([
                        this.gridData[event.previousIndex].id,
                        this.gridData[event.currentIndex].id,
                    ]);
    ...
    detailClick() {
        const masterRow = document.getElementsByClassName(
            'k-master-row'
        ) as HTMLCollectionOf<HTMLElement>;
        masterRow[0].style.display = 'table-row';
        const detailRow = document.getElementsByClassName(
            'k-detail-row'
        ) as HTMLCollectionOf<HTMLElement>;
    ...
    userChangeEnd(event, index, data) {
        ...
        if (data[index + 1]) {
            ...
            const cssStyle1 =
                span1.nextElementSibling?.children[3].attributes[3]
                    .ownerElement;
        ...
        if (data[index - 1] !== undefined) {
            ...
            const cssStyle2 =
                span2.nextElementSibling?.children[3].attributes[3]
                    .ownerElement;
    ...
    addHOS(data, hosType) {
        ...
        setTimeout(() => {
            const span = document.getElementById(
                'valueSpan_' + (data.length - 1)
            );
            const button = document.getElementById(
                'buttonId_' + (data.length - 1)
            );
            const cssStyle =
                span.nextElementSibling?.children[3].attributes[3].ownerElement;
            span.style.width = cssStyle.clientWidth + 'px';
                ...
                if (event.previousContainer.id !== event.container.id) {
                    if (
                        ...
                        setTimeout(() => {
                            const span = document.getElementById('valueSpan_0');
                            const button =
                                document.getElementById('buttonId_0');
                            const cssStyle =
                                span.nextElementSibling?.children[3]
                                    .attributes[3].ownerElement;
                            span.style.width = cssStyle.clientWidth + 'px';
                            button.style.marginLeft = data[0].end / 3.8 + 'px';
                        });
                        ...
                        if (event.previousIndex === 0) {
                            if (
                                event.previousContainer.data[0].flag === 'off'
                            ) {
                                tempObj = {
                                    start: event.previousContainer.data[0]
                                        .start,
                            ...
                            setTimeout(() => {
                                const span =
                                    document.getElementById('valueSpan_0');
                                const button =
                                    document.getElementById('buttonId_0');
                                const cssStyle =
                                    span.nextElementSibling?.children[3]
                            ...
                            if (
                                event.previousContainer.data[1].flag === 'off'
                            ) {
                                tempObj = {
                                    start: event.container.data[0].start,
                            ...
                            setTimeout(() => {
                                const span =
                                    document.getElementById('valueSpan_1');
                                const button =
                                    document.getElementById('buttonId_1');
                                const cssStyle =
                                    span.nextElementSibling.children[3]
                                        .attributes[3].ownerElement;
                                span.style.width = cssStyle.clientWidth + 'px';
    ...
    saveHosData(data, driverId, dataId) {
        ...
        this.waitForResponse = true;
        this.loadService
            .saveHos(JSON.stringify(saveData), driverId)
            .pipe(takeUntil(this.destroy$))
            .subscribe(
                (response) => {
    ...
    public handleSignalMessage(resp) {
        switch (resp.type) {
            ...
            case 'tableChange':
                if (
                    (resp.user_id != this.user.id &&
                        resp.index == this.gridIndex) ||
    ...
    handleKeyboardEvent(event: any) {
        if (event.target && event.target.className) {
            if (
                ...
                if (
                    !event.target.className.includes('btn-transparent') &&
                    !event.target.parentNode.className.includes(
                        'btn-transparent'
                    )
                ) {
        ...
        [
            ...
            if (
                data[res]
                    ...
                    .toLowerCase()
                    .includes(
                        this.highlightingWords[0]?.text.toLowerCase().trim()
                    )
            ) {
                founded = true;
            }
    ...
    filterItemData() {
        if (this.highlightingWords.length && this.highlightingWords[0].text) {
            ...
            this.savedMainGridData.sort((a, b) => {
    ...
    public openedRoutes(data: any) {
        if (data.isShown) {
            this.loadOpenedIndx = data.indx;
        } else {
            this.loadOpenedIndx = -1;
        }
    }
    ...
    private sortItems(do_rotate: boolean): void {
        if (do_rotate) {
            this.sortService.sortDirection =
                rotate[this.sortService.sortDirection];
        }
        if (!this.isBoardLocked) {
