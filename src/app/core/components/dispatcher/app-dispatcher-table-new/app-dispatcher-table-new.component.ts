...
export class AppDispatcherTableNewComponent implements OnInit, OnChanges {
               ...
               .subscribe((data) => {
                  data.map((item) => {
                     ...
                     if (item.driverAddress) {
                        item.driverAddress = JSON.parse(item.driverAddress);
                     }
                     if (item.route) {
                        item.route = JSON.parse(item.route);
                     }
                     if (item.truckloadJson) {
                        item.truckloadJson = JSON.parse(item.truckloadJson);
                     }
   ...
   public getDistance(origins: string, destinations: string, waypoint: any[]) {
      ...
      directionsService.route(request, (response, status) => {
         if (status === google.maps.DirectionsStatus.OK) {
            ...
            this.distance =
               (totalDistance * 0.00062137).toFixed(0).toString() + 'miles';
         }
      });
   }
            ...
            .subscribe((res) => {
               ...
               res.forEach((elem: any, index: any) => {
                  if (!elem.error) {
                     ...
                     this.savedMainGridData = this.savedMainGridData.filter(
   ...
   addStatus(event: any, item: any, rowInd: number) {
      ...
      if ([5000, 5100, 5200, 6000].includes(event.id)) {
         ...
         if (event.id == 5200) {
            ...
            const finded_route = JSON.parse(
               JSON.stringify(
                  this.findLocationInRoutes(rowInd, 'delivery', load_count)
               )
            );
            data.inDateTime = new Date();
            data.pointCity = finded_route.pointCity;
   ...
   locationChangeBlur(rowInd: number) {
      setTimeout(() => {
         ...
         if (this.gridData[rowInd].id) {
            ...
            this.gridData[rowInd] = JSON.parse(JSON.stringify(this.emptyRow));
   ...
   userChangeEnd(event, index, data) {
      ...
      if (data[index + 1]) {
         ...
         const cssStyle1 =
            span1.nextElementSibling?.children[3].attributes[3].ownerElement;
      ...
      if (data[index - 1] !== undefined) {
         ...
         const cssStyle2 =
            span2.nextElementSibling?.children[3].attributes[3].ownerElement;
         ...
         case 3:
            if (event.previousContainer.id !== event.container.id) {
               if (
                  ...
                  let tempObj = {};
                  if (
                     event.previousContainer.data[event.previousIndex].flag ===
                     'off'
                  ) {
                     tempObj = {
   ...
   public handleSignalMessage(resp) {
      switch (resp.type) {
         ...
         case 'tableChange':
            if (
               (resp.user_id != this.user.id && resp.index == this.gridIndex) ||
