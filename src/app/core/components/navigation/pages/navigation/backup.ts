...
export class NavigationComponent implements OnInit, OnDestroy {
    ...
    public bottomNavigation: Navigation[] = NavigationDataConstants.footerNavigation;
    ...
    public onRouteEvent(subroute: NavigationSubRoutes, isFooterNavigation?:boolean): void {
        localStorage.removeItem('footer_active');
        const navigation = isFooterNavigation ? this.bottomNavigation : this.navigation;
        let index = navigation.findIndex(
        ...
        if (Array.isArray(subroute.routes)) {
            this.activationSubRoute(index, subroute, isFooterNavigation);
        } else if (index > -1) {
            this.activationMainRoute(index, isFooterNavigation);
        }
    }
    ...
    private activationSubRoute(
        ...
        if (index === this.isActiveSubrouteIndex) { 
            ...
            navigation[index].isRouteActive =
                !navigation[index].isRouteActive;

            if (subroute.activeRouteFlegId === navigation[index].id) {
                this.isActiveSubroute = true;
                this.activeSubrouteFleg = true;
            }

            if (!navigation[index].isRouteActive) {
                this.isActiveSubroute = false;
                navigation[index].isSubrouteActive = true;
            } else {
                this.isActiveSubroute = true;
                navigation[index].isSubrouteActive = false;
            }
        }

        if (index !== this.isActiveSubrouteIndex) {
            this.bottomNavigation.forEach((nav) => (nav.isRouteActive = false));
            this.navigation.forEach((nav) => (nav.isRouteActive = false));
            // this.isActiveSubroute = true;
            this.activeSubrouteFleg = false;
            if (this.isActiveSubrouteIndex != -1 && navigation[this.isActiveSubrouteIndex]) {
                navigation[this.isActiveSubrouteIndex].isSubrouteActive =
                    false;
            }
            this.isActiveSubrouteIndex = index;
            navigation[index].isRouteActive = true;
        }
        this.isActiveFooterRoute = false;
    }

    private activationMainRoute(index: number,
        isFooterNavigation?:boolean): void {
        this.navigationService.setValueWhichNavIsOpen(true);
        this.disableRoutes();
        const navigation = isFooterNavigation ? this.bottomNavigation : this.navigation;
        navigation[index].isRouteActive = true;
