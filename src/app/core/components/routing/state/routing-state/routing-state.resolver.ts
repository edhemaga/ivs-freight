...
import { RoutingStateState, RoutingStateStore } from './routing-state.store';
            ...
            .pipe(
                tap(([mapList]) => {
                    mapList.pagination.data.map((item) => {
                        const mapData = {...item, type: 'map', fakeId: parseInt([1, item.id].join(''))};
                        ...
                        this.routingService
                            ...
                            .subscribe((routes: any) => {
                                routes.pagination.data.map((route) => {
                                    const routeData = {...route, type: 'route', mapId: item.id, fakeId: parseInt([2, route.id].join(''))};
