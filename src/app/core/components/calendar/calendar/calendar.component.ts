...
export class CalendarComponent implements OnInit {
  ...
  setCalendarOptions(ev, type, headerInfo) {
    ...
    this.calendarOptions = {
      ...
      eventDidMount: function (info) {
        var dotEl = <HTMLBodyElement>(
          info.el.getElementsByClassName('fc-daygrid-event-dot')[0]
        );
        if (dotEl) {
  ...
  public checkCalendarTitle() {
    ...
    if (this.currentCalendarView === 'day') {
      this.calendarTitle = moment(calendarApi.currentData.currentDate).format(
        'MMMM D, YYYY'
      );
    } else if (
  ...
  changeCalendarView(view) {
    if (this.currentCalendarView == view) {
      return false;
    }
  ...
  public changeViewCalendar(view: any) {
    ...
    if (this.calendarGridView == 'timeGridWeek') {
      var topLeftColumn = <HTMLElement>(
        document.querySelector('.fc-timegrid-axis')
      );
      topLeftColumn.innerHTML =
        '<div class="fc-scrollgrid-sync-inner"><a class="fc-col-header-cell-cushion">TIME</a></div>';
    }
  }
