   ...
   implements OnInit, OnDestroy, ControlValueAccessor
{
   ...
   public manipulateWithInput(event: KeyboardEvent): boolean {
      ...
      if (['address'].includes(this.inputConfig.name.toLowerCase())) {
         if (
            /^[A-Za-z0-9\s.&/,_-]*$/.test(String.fromCharCode(event.charCode))
         ) {
            this.disableConsecutivelySpaces(event);
            return true;
         } else {
      ...
      if (
         ['truck-trailer-model'].includes(this.inputConfig.name.toLowerCase())
      ) {
         if (
            this.inputService
   ...
   private pasteCheck(pastedText: any, regexType?: RegExp) {
      ...
      if (
         ['phone', 'ein', 'ssn'].includes(this.inputConfig.name.toLowerCase())
      ) {
