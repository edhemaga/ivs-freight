 ...
 */
@Injectable()
export class CalendarLeftStrategy implements VirtualScrollStrategy {
  ...
  private updateRenderedRange(viewport: CdkVirtualScrollViewport) {
    ...
    if( this.calendarService.selectedScroll === 'left' ){
      this.index$.next({indx: firstVisibleIndex, scrollOffset: offset, cycleSize: FULL_SIZE, type: "left"});
    }
  }
}
