...
export class CustomToastMessagesComponent extends Toast implements OnInit {
  ...
  createTitleBasedOnHttpRequest() {
    ...
    const item = this.apiConfObj.find(
      (item) => item.api === apiEndPoint || apiEndPoint.indexOf(item.api) > -1
    );
    ...
    switch (this.actionType) {
      ...
      case 'DRIVER':
        ...
        if (this.httpRequest.method == 'PUT') {
          if (apiEndPoint.indexOf('status') > -1) {
            if (active == 1) {
              this.actionTitle =
                this.toastrType == 'toast-error' ? 'DEACTIVE' : 'DEACTIVATED';
            } else {
              this.actionTitle =
                this.toastrType == 'toast-error' ? 'ACTIVE' : 'ACTIVATED';
            }
          }
        }
      ...
      case 'RATE':
        ...
        if (!likedStatus && !dislikedStatus) {
          this.actionTitle =
            this.toastrType == 'toast-error' ? 'REMOVE RATE' : 'REMOVED RATE';
        }
        if (likedStatus || dislikedStatus) {
          this.actionTitle =
      ...
      case 'CDL':
        ...
        if (this.httpRequest.body) {
          ...
          if (this.httpRequest.method == 'POST') {
            this.actionTitle =
              this.toastrType == 'toast-error' ? 'ADD NEW' : 'ADDED NEW';
          } else if (this.httpRequest.method == 'PUT') {
            this.actionTitle =
              this.toastrType == 'toast-error' ? 'ACTIVATE' : 'ACTIVATED';
          }
      ...
      case 'OWNER':
        ...
        if (apiEndPoint.indexOf('verifyowner') > -1) {
          ...
          this.actionTitle =
            this.toastrType == 'toast-error' ? 'VERIFY' : 'VERIFIED';
        }
      ...
      case 'INSPECTION':
        let inspectionDate = this.httpRequest.body?.issueDate
          ? moment(this.httpRequest.body?.issueDate).format('MM/DD/YY')
          : '';
      ...
      case 'INSURANCE POLICY' : 
        let producerName = this.httpRequest.body?.producerName ? this.httpRequest.body?.producerName : this.DetailsDataService.cardMainTitle;
        this.message = producerName;
      break;
      case 'FACTORING COMPANY' : 
        let factoringCompanyName = this.httpRequest.body?.name ? this.httpRequest.body?.name : this.DetailsDataService.cardMainTitle;
        this.message = factoringCompanyName;
      break;
      case 'PARKING' :
        let parkingName = this.httpRequest.body?.name ? this.httpRequest.body?.name : this.DetailsDataService.cardMainTitle;
        this.message = parkingName;
      break;
      case 'OFFICE' : 
        let officeName = this.httpRequest.body?.name ? this.httpRequest.body?.name : this.DetailsDataService.cardMainTitle;
        this.message = officeName;
      break;
      case 'TERMINAL' : 
        let terminalName = this.httpRequest.body?.name ? this.httpRequest.body?.name : this.DetailsDataService.cardMainTitle;
        ...
        switch (this.actionType) {
            ...
            case 'CDL':
                ...
                if (this.httpRequest.body) {
                    ...
                    if (!this.httpRequest.body?.cdlNumber) {
                        let cdlId = lastVal;
                        this.DetailsDataService.mainData?.cdls?.map(
                            (item: any) => {
                                if (item.id == cdlId) {
                                    cdlNum = item.cdlNumber;
                                }
